...
from webapp.api.validation import ChatLogPathRequest, EmptyRequest, validate_data, validate_json, validate_user_id
...
def like(chat_log_id):
    ...
    response = chat_service.like(chat_log_id=chat_log_id, user_id=user_id)
...
def dislike(chat_log_id):
    ...
    validate_json(EmptyRequest)
    validate_data(ChatLogPathRequest, {"chat_log_id": chat_log_id})
    response = chat_service.dislike(chat_log_id=chat_log_id, user_id=user_id)
    return success(response)
